...
get_build_log_url() {
    ...
    if [ -z "$log_name" ]; then
        echo "https://github.com/VOS/build-logs/tree/master/$(hostname)"
    else
        echo "https://github.com/VOS/build-logs/tree/master/$log_name"
    fi
}
...
export VANIR_BUILD_LOG_CMD="qrexec-client-vm 'dom0' vanirbuilder.BuildLog >$log_service_output_file"
