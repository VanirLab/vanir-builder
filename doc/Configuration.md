...
("vanir-r1/"), or real prefix ("VanirLab/vanir-")
...
For details see http://wiki.vanir-os.org/trac/ticket/581
Available values: "yes" or "no"

### REMOTE_COMPONENTS_<git-remote>
> Default: no value

track only selected Vanir components for
...
vanir-builder and template-builder plugins (can be a list
...
base Vanir OS build you probably want to enable 'builder-rpm' plugin.
...
will be copied to vanir-builder, so it shouldn't rely directly on other files
...
Use Vanir packages repository (yum.vanir-os.org/deb.vanir-os.org) to satisfy
build dependents. Set to target version of Vanir you are building packages for
(like "3.1", "3.2" etc).
...
Example versions with devel build number:
 ...
 - `vanir-core-dom0-3.2.13-1.13.fc23.x86_64` (`.13` is the additional version here)
 - `vanir-gpg-split_2.0.24-1+deb8u1+devel1` (`+devel1` is the additional version here)

### USE_DIST_BUILD_TOOLS
> Default: empty

When set to `1`, builder plugins (builder-rpm, builder-debian, etc) will use upstream tools to prepare build environment (`mock` for Fedora, `pbuilder` for Debian), instead of custom code handling chroot preparation. This is recommented way of building, but older packages (Vanir 3.2) are incompatible with this mode.
...
print verbose messages about vanir-builder itself - set "1" to use it
...
========================================
    ...
    GIT_BASEURL=/home/user/vanir-src/
    GIT_PREFIX=
    GIT_SUFFIX=
