...
INSTALLER_COMPONENT ?= installer-vanir-os
BACKEND_VMM ?= xen
...
.ONESHELL:
help:
	@echo "make vanir            -- download and build all components"
	@echo "make vanir-dom0       -- download and build all dom0 components"
	@echo "make vanir-vm         -- download and build all VM components"
	...
	@echo "make vanir-os-iso     -- same as \"make get-sources vanir sign-all iso\""
	...
	@echo "make show-authors     -- list authors of vanir code based on commit log of each component"
...
vanir:: build-info $(COMPONENTS_NO_BUILDER)

vanir-dom0:: build-info
vanir-dom0:: $(addsuffix -dom0,$(COMPONENTS_NO_TPL_BUILDER))

vanir-vm:: build-info
vanir-vm:: $(addsuffix -vm,$(COMPONENTS_NO_TPL_BUILDER))

vanir-os-iso: get-sources vanir sign-all iso
...
clean-rpms:: clean-installer-rpms
	@for dist in $(DISTS_ALL); do \
		echo "Cleaning up rpms in vanir-packages-mirror-repo/$$dist/..."; \
		sudo rm -rf vanir-packages-mirror-repo/$$dist || true ;\
	done
...
distclean: clean-all
	...
	find $(BUILDER_DIR)/vanir-packages-mirror-repo/* -maxdepth 0 -type d -exec rm -rf {} \;
...
windows-image-extract:
	...
	( shopt -s nullglob; cd mnt; cp --parents -rft .. vanir-src/*/*.{msi,exe} )
	for REPO in $(GIT_REPOS); do \
		[ $$REPO == '.' ] && break; \
		if [ -r $$REPO/Makefile.builder ]; then \
